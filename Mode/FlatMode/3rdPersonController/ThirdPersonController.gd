...
func _process(delta) :
	...
	$Mannequiny/SpringArm.position = position
...
func handle_rotation(delta):
	...
	velocity = velocity.rotated(Vector3.UP, $Mannequiny/SpringArm.rotation.y)
	# We sync the model rotation in order to also rotate the character in the direction we are moving
	$Mannequiny.rotation.y = $Mannequiny/SpringArm.rotation.y
