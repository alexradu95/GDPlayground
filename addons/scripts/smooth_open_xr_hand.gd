...
func _ready():
	var OneEuroFilter = load("res://addons/scripts/one_euro_filter.gd")
	var args: Dictionary = {
...
func _process(delta):
	if source_skeleton:
		...
		set_global_transform(Transform3D(source_skeleton.get_global_transform().basis, xr_origin.global_transform.origin + Vector3(x, y, z)))
